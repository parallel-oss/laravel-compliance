...
class GenerateVantaData extends Command
{
    protected $signature = 'security:generate-vanta-data
        ...
        {--control-enum-output= : Optional ComplianceControl enum file to regenerate from curated controls}';
    ...
    public function handle(): int
    {
        ...
        if ($this->option('control-enum-output')) {
            $this->writeComplianceControlEnum((string) $this->option('control-enum-output'), $data['internal-controls']);
        }
     ...
     */
    private function writeComplianceControlEnum(string $path, array $controls): void
    {
        ...
        foreach ($controls as $control) {
            $slug = (string) $control['vanta_id'];
            $name = (string) ($control['name'] ?? $slug);
            $caseName = $this->enumCaseName($name);
            $description = $name;
        ...
        $contents .= "/**\n";
        $contents .= " * @generated from package compliance seed data\n";
        $contents .= " */\n";
        $contents .= "enum ComplianceControl: string implements Control\n";
        $contents .= "{\n";
...
PHP;
    ...
    {
        $name = preg_replace('/[^A-Za-z0-9]+/', ' ', $externalId) ?: $externalId;
